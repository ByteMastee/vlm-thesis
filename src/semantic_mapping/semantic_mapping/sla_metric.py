"""
Semantic Label Accuracy (SLA) Metric
======================================
SLA = Correctly labelled objects / Total Ground Truth objects

- Position matching uses Hungarian algorithm.
- Label matching uses cosine similarity between sentence-transformer
  embeddings with a configurable threshold.
"""

import os
import json
import numpy as np
from scipy.optimize import linear_sum_assignment
from sentence_transformers import SentenceTransformer

# =============================================================================
# USER INPUTS — change these for every run
# =============================================================================

GT_STRING     = "chair_1:-3.0:2.0,chair_2:-3.5:-2.5,table:2.0:2.5,couch:3.5:0.0"

YOLO_PATH     = "/root/UVC_ws/vf_robot_model_ros2/Final_Output/PreFinal/E1_Path1_1/E1_Path1_1_object_stack.json"
YOLO_VLM_PATH = "/root/UVC_ws/vf_robot_model_ros2/Final_Output/PreFinal/E1_Path1_1/E1_Path1_1_vlm_object_stack.json"
VIT_VLM_PATH  = "/root/UVC_ws/vf_robot_model_ros2/Final_Output/ViT/Env1_Path1/Env1_Path1_vit_vlm_object_stack.json"

RUN_NAME      = "E1_Path1"   # used in output filename

# =============================================================================
# FIXED — do not change
# =============================================================================

OUTPUT_DIR    = "/root/UVC_ws/vf_robot_model_ros2/Final_Output/SLA_Metrics"
MODEL_PATH    = "/root/all-MiniLM-L6-v2"
THRESHOLD     = 0.6

# =============================================================================


def parse_gt(gt_string):
    gt = {}
    for entry in gt_string.split(','):
        entry = entry.strip()
        parts = entry.split(':')
        label = parts[0].strip()
        x     = float(parts[1])
        y     = float(parts[2])
        gt[label] = (x, y)
    return gt


def load_object_stack_with_labels(path):
    with open(path, 'r') as f:
        data = json.load(f)
    objects = {}
    for key, val in data.items():
        label = val.get('vlm_label', key)
        objects[key] = {
            'x':     float(val['x']),
            'y':     float(val['y']),
            'label': label
        }
    return objects


def cosine_similarity(vec1, vec2):
    dot  = np.dot(vec1, vec2)
    norm = np.linalg.norm(vec1) * np.linalg.norm(vec2)
    if norm == 0:
        return 0.0
    return float(dot / norm)


def compute_sla(gt, predicted, model):
    gt_labels  = list(gt.keys())
    pred_keys  = list(predicted.keys())

    if len(pred_keys) == 0:
        return 0.0, [], gt_labels

    gt_positions   = np.array([gt[l] for l in gt_labels])
    pred_positions = np.array([(predicted[k]['x'], predicted[k]['y']) for k in pred_keys])

    n_gt   = len(gt_labels)
    n_pred = len(pred_keys)
    cost   = np.zeros((n_gt, n_pred))

    for i in range(n_gt):
        for j in range(n_pred):
            cost[i, j] = np.linalg.norm(gt_positions[i] - pred_positions[j])

    row_ind, col_ind = linear_sum_assignment(cost)

    gt_embeddings  = {l: model.encode(l) for l in gt_labels}
    matched_gt_idx = set()
    details        = []

    for r, c in zip(row_ind, col_ind):
        gt_label   = gt_labels[r]
        pred_key   = pred_keys[c]
        pred_label = predicted[pred_key]['label']
        dist       = round(cost[r, c], 4)

        pred_emb = model.encode(pred_label)
        gt_emb   = gt_embeddings[gt_label]
        sim      = round(cosine_similarity(pred_emb, gt_emb), 4)
        correct  = sim >= THRESHOLD

        details.append({
            'pred_key':   pred_key,
            'pred_label': pred_label,
            'gt_label':   gt_label,
            'distance_m': dist,
            'similarity': sim,
            'correct':    correct
        })
        matched_gt_idx.add(r)

    correctly_labelled = sum(1 for d in details if d['correct'])
    sla                = round(correctly_labelled / len(gt_labels), 4)
    unmatched_gt       = [gt_labels[i] for i in range(len(gt_labels))
                          if i not in matched_gt_idx]

    return sla, details, unmatched_gt


def main():
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    print(f'Loading sentence-transformer model from: {MODEL_PATH}')
    model = SentenceTransformer(MODEL_PATH)

    gt = parse_gt(GT_STRING)

    pipelines = {
        'YOLO':     load_object_stack_with_labels(YOLO_PATH),
        'YOLO_VLM': load_object_stack_with_labels(YOLO_VLM_PATH),
        'VIT_VLM':  load_object_stack_with_labels(VIT_VLM_PATH)
    }

    results = {}

    for name, predicted in pipelines.items():
        sla, details, unmatched_gt = compute_sla(gt, predicted, model)

        results[name] = {
            'SLA':          sla,
            'threshold':    THRESHOLD,
            'N_gt':         len(gt),
            'N_detected':   len(predicted),
            'N_matched':    len(details),
            'N_correct':    sum(1 for d in details if d['correct']),
            'unmatched_gt': unmatched_gt,
            'details':      details
        }

        print(f'\n── {name} ──')
        print(f'  SLA       : {sla}  ({sum(1 for d in details if d["correct"])}/{len(gt)} correct)')
        print(f'  Threshold : {THRESHOLD}')
        for d in details:
            status = 'CORRECT' if d['correct'] else 'WRONG'
            print(f'  [{status}] pred="{d["pred_label"]}" <-> gt="{d["gt_label"]}"'
                  f'  sim={d["similarity"]}  dist={d["distance_m"]}m')
        if unmatched_gt:
            print(f'  Unmatched GT : {unmatched_gt}')

    output_path = os.path.join(OUTPUT_DIR, f'{RUN_NAME}_sla_results.json')
    with open(output_path, 'w') as f:
        json.dump(results, f, indent=2)
    print(f'\nSLA results saved to: {output_path}')


if __name__ == '__main__':
    main()