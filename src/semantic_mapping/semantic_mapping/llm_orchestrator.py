...
4. If the instruction mentions a relative direction (in front of, behind, left of, right of), extract only the object name and the direction word separately.
5. For the GOAL line, copy the x and y values exactly as plain numbers from the map above. Do not compute, add, subtract, or modify the values in any way.
                ...
                for part in goal_part.split(","):
                    ...
                    try:
                        ...
                        self.get_logger().warn(
                            f'Could not parse value from: {part} — LLM returned malformed GOAL.'
                        )
                        return None, None, None, None
