...
class VlmLabelNodeVit(Node):
    ...
    def _get_vlm_label(self, obj_path, env_context):
        object_prompt = (
            ...
            f'Rules: '
            f'1. If the crop clearly shows a real indoor object (furniture, appliance, equipement, document or any item), '
            f'reply with a specific short label (e.g. black chair, wooden table, green folder, computer monitor). '
            f'Do not reply with a single generic word like "chair" or "table" alone. '
            f'2. If the crop contains only a flat structural surface with no object present at all '
            f'(only wall, only floor, or only ceiling with nothing else), '
            f'reply with the single word: none '
            f'3. If the crop is too blurry or too small to identify any object at all, '
            f'reply with the single word: none '
