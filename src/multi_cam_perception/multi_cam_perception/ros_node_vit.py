...
from multi_cam_perception.sam2_map_node import SAM2MapNode
...
class RosBridgeNodeVit(Node):
    def __init__(self):
        ...
        self.declare_parameter('image_topic_left',    '/fisheye_left/fisheye_left/image_raw')
        ...
        self.create_subscription(CameraInfo, cam_info_topic_left, self.cam_info_left_cb, 10)
        ...
        self.sync.registerCallback(self.dual_image_cb)
    ...
    def dual_image_cb(self, msg_front, msg_left):
        ...
        if not self.is_calibrated or not self.is_calibrated_left:
            return
        ...
        self.get_logger().info(
            f'[{self.run_name}] Frame {self.frame_count} processed (dual) | '
        ...
        all_rays = []
        if frame_rays:
            all_rays += [(o, r, self.ray_length) for o, r in frame_rays]
        if frame_rays_left:
            all_rays += [(o, r, self.ray_length) for o, r in frame_rays_left]
    ...
    def process(self):
        ...
        # Unload SAM2 before triggering VLM to free VRAM
        self._unload_sam2()
